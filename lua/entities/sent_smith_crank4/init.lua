...
function ENT:Initialize()
	self:SetModel( "models/hoxii/smith/crank4.mdl" )
...
function ENT:Use( activator, caller, type, value )
	...
	if ( IsValid( self.LastUser ) ) then return end		-- Someone is already using this rotator
...
--
-- Makes the rotator trigger the keys
--
	...
	if ( bEnable ) then
		...
		if IsValid(self.smith) then
			net.Start("smithInt-ControlSound")
				...
				net.WriteString("hoxii/smith/crank2.wav")
			net.Broadcast()
		end
	else
		self:SetOn( false )
