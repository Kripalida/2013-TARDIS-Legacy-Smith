...
function ENT:Initialize()
	self:SetModel( "models/hoxii/smith/lever.mdl" )
...
function ENT:Toggle( bEnable, ply )
	if ( bEnable ) then
		...
		if IsValid(self.smith) then
			net.Start("smithInt-ControlSound")
				...
				net.WriteString("hoxii/smith/lever.wav")
			net.Broadcast()
		end
	else
		self:SetOn( false )
	...
	if IsValid(interior) and IsValid(smith) then
		...
		if tobool(GetConVarNumber("smith_advanced"))==true and (interior.flightmode==1 or interior.flightmode==2 or interior.flightmode==3) and interior.step==7 then
			interior:UpdateAdv(ply, true)
		else
			interior:UpdateAdv(ply, false)
			local success=smith:TogglePhysLock()
			if success then
				if smith.physlocked then
