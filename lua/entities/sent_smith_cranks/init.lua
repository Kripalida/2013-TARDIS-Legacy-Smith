...
function ENT:Initialize()
	self:SetModel( "models/hoxii/smith/cranks.mdl" )
...
function ENT:Toggle( bEnable, ply )
	if ( bEnable ) then
		...
		if IsValid(self.smith) then
			net.Start("smithInt-ControlSound")
				...
				net.WriteString("hoxii/smith/cranks.wav")
			net.Broadcast()
		end
	else
		self:SetDir( false )
	...
	if tobool(GetConVarNumber("smith_advanced"))==true and IsValid(interior) then
		if (interior.flightmode==1 or interior.flightmode==2 or interior.flightmode==3) and interior.step==5 then
