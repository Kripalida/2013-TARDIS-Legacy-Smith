...
function ENT:Initialize()
	self:SetModel( "models/hoxii/smith/switch2.mdl" )
...
function ENT:Toggle( bEnable, ply )
	if ( bEnable ) then
		...
		if IsValid(self.smith) then
			net.Start("smithInt-ControlSound")
				...
				net.WriteString("hoxii/smith/switch2.wav")
			net.Broadcast()
		end
	else
		self:SetOn( false )
	...
	if IsValid(self.smith) then
		local success=self.smith:TogglePhase()
		if success then
			if self.smith.visible then
				ply:ChatPrint("TARDIS now visible.")
			elseif not self.smith.visible then
