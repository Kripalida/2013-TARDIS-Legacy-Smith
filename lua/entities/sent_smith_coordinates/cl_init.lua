...
net.Receive("smithInt-Locations-GUI", function()
	local interior=net.ReadEntity()
	local smith=net.ReadEntity()	
	local coordinates=net.ReadEntity()	
	local locations={}
	...
	local function TransmitCoordinates()
		...
		if IsValid(interior) and IsValid(smith) and (not smith.moving or smith.invortex) and pos and ang then
			net.Start("smithInt-Locations-Send")
				net.WriteEntity(interior)
				net.WriteEntity(smith)
				net.WriteEntity(coordinates)
	...
	button.DoClick = function( button )
		if selected then
			selected.pos=smith:GetPos()
			selected.ang=smith:GetAngles()
			UpdateTextBoxes()
			changed=true
		else
			LocalPlayer():ChatPrint("No location selected.")
