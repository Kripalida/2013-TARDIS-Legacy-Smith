...
function ENT:Initialize()
	self:SetModel( "models/hoxii/smith/lever5.mdl" )
...
function ENT:Toggle( bEnable, ply )
	if ( bEnable ) then
		...
		if IsValid(self.smith) then
			net.Start("smithInt-ControlSound")
				...
				net.WriteString("hoxii/smith/lever5.wav")
			net.Broadcast()
		end
	else
		self:SetOn( false )
	...
	if IsValid(self.smith) then
		local success=self.smith:ToggleLongFlight()
		if success then
			if self.smith.longflight then
				ply:ChatPrint("TARDIS long-flight enabled.")
			elseif not self.smith.longflight then
