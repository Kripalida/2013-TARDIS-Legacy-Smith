...
util.AddNetworkString("smithInt-SetSkyCamera")
...
function ENT:PlayerEnter(ply)
	...
	net.Start("smithInt-SetSkyCamera")
	...
	elseif not self.controller and IsValid(self.smith) and ((self.smith.isomorphic and ply==self.owner) or not self.smith.isomorphic) and self.smith.power then
		self.controller=ply
...
function ENT:PlayerExit(ply,override)
	...
	if ply.smithint_pos and ply.smithint_ang then
	...
	if self.controller and self.controller==ply then
		self.controller=nil
		if #self.occupants>0 and IsValid(self.smith) and not self.smith.isomorphic and self.smith.power then
	...
	if self.controller and IsValid(self.controller) and self.controller:IsPlayer() then		
		...
		if self.controller:KeyDown(IN_ATTACK) then
			if CurTime()>self.setcur then
				...
				ang=Angle(0,ang.y,0)
				if IsValid(self.smith) and self.smith.invortex then
					self.smith:SetDestination(pos,ang)
				else
					self.hitpos=pos
					self.hitang=ang
				end
